    ...
    <div class="d-flex">
        <div class="card flex-fill">
            ...
            <form enctype="multipart/form-data">
                ...
                <div class="row">
                        ...
                        <div class="col-3 ">
                            <div class="from-group">
                                @if ($certificate->type == 1)
                                    <label>IELTS</label>
                                    <input type="text" value="{{ $certificate->ball }}">
                                @elseif ($certificate->type == 2)
                                    <label>CEFR</label>
                                    <input type="text"
                                        value="{{ App\Models\Student::DEGREE[$certificate->degree] }}">
                                @elseif ($certificate->type == 3)
                                    <label for="">Bali bo'lmidibuni</label>
                                @endif
                            </div>
                        </div>
                    </div>
                    <div class="col-6 " >
                        <div class="card-body">
                            ...
                            <select wire:change="updateType()" wire:model="type" class="form-select" name="type"
                                id="">
                                <option selected>Certificatni tanlang</option>

                                @foreach (App\Models\Student::TYPES as $id => $type)
                                    <option {{ $certificate->type == $id ? 'selected' : '' }}
                                        value="{{ $id }}">
                                        {{ $type }}
                                    </option>
                                @endforeach
                            ...
                            <div class="card-body">
                                ...
                                <select wire:model="degree" class="form-select" name="degree" id="">
                                    ...
                                    @foreach (App\Models\Student::DEGREE as $id => $type)
                                        <option @if ($certificate->degree == $id) selected @endif
                ...
                <button wire:click="editCertificate" class="btn btn-primary mt-3" type="button">
